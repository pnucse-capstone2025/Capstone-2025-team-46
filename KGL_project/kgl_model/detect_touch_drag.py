...
MODEL_PATH = os.path.join(ROOT_DIR, "lstm_ae_touch_drag.pth")
SCALER_PATH = os.path.join(ROOT_DIR, "scaler_touch_drag.pkl")
OUTPUT_JSON_PATH = "drag_anomalies.json"

SEQ_LEN = 20
THRESHOLD_MULTIPLIER = {"lower": 0.5, "upper": 1.1}
...
def ensure_drag_features(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty: return df
    df = df.copy()
    for c in ["start_x", "start_y", "end_x", "end_y", "total_distance", "duration", "touch_x", "touch_y"]:
        if c in df.columns: df[c] = pd.to_numeric(df[c], errors="coerce")
    if "dx" not in df.columns:
        if {"end_x", "start_x"}.issubset(df.columns): df["dx"] = (df["end_x"] - df["start_x"]).astype(float)
        elif "touch_x" in df.columns: df["dx"] = df["touch_x"].diff().fillna(0.0).astype(float)
        else: df["dx"] = 0.0
    if "dy" not in df.columns:
        if {"end_y", "start_y"}.issubset(df.columns): df["dy"] = (df["end_y"] - df["start_y"]).astype(float)
        elif "touch_y" in df.columns: df["dy"] = df["touch_y"].diff().fillna(0.0).astype(float)
        else: df["dy"] = 0.0
    if "speed" not in df.columns:
        speed = None
        if {"total_distance", "duration"}.issubset(df.columns):
            dur_s = (df["duration"].astype(float) / 1000.0).replace(0, np.nan)
            with np.errstate(divide="ignore", invalid="ignore"):
                speed = (df["total_distance"].astype(float) / dur_s).replace([np.inf, -np.inf], np.nan)
        if speed is None or (isinstance(speed, pd.Series) and speed.isna().all()):
            speed = np.sqrt(df["dx"].astype(float)**2 + df["dy"].astype(float)**2)
        df["speed"] = speed.fillna(0.0).astype(float)
    return df
...
def process_file(file_path, model, scaler, num_cols, q01, q99, device):
    ...
    # [수정] 정규식 패턴을 non-capturing group으로 변경하여 pandas 경고 메시지 방지
    df = df[df["touch_event"].str.contains(r"(?:drag|move|swipe)", case=False, na=False, regex=True)].copy()
    if df.empty: return []
    
    df.sort_values("ts", inplace=True); df.reset_index(drop=True, inplace=True); df = ensure_drag_features(df)
    ...
    for i in anomaly_indices:
        ...
        report = {
            "modality": "touch_drag", "file": os.path.basename(file_path),
...
def main():
    ...
    print("="*20 + " Touch Drag Anomaly Detection " + "="*20)
    all_anomalies = []
